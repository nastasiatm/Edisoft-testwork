        ...
        <Document-DespatchAdvice>
            <DespatchAdvice-Header>
                <DespatchAdviceNumber>
                    <xsl:value-of select="//Document-Header/НомерДок"/>
                </DespatchAdviceNumber>
                <DespatchAdviceDate>
                ...
                <BuyerOrderNumber>
                    <xsl:value-of select="substring(//Document-Header/Основание, 1, 13)"/>
                </BuyerOrderNumber>
            ...
            <Document-Parties>
                <Sender>
                    <ILN>
                        <xsl:value-of select="normalize-space(//Фирма/@ILN)"/>
                    </ILN>
                </Sender>
                <Receiver>
                    <ILN>
                ...
                <Packing-Sequence>
                    <xsl:for-each select="//Line-Item">
                        <Line>
                            ...
                            <UnitNetPrice>
                                <xsl:value-of select="format-number(./Цена, '.00')"/>
                            </UnitNetPrice>
                            <xsl:variable name="priceVAT">
                                <xsl:value-of select="./Цена * (1 + ./СтавкаНДС*0.01)"/>
                            </xsl:variable>
                            <UnitGrossPrice>
                                <xsl:value-of select="format-number($priceVAT, '.00')"/>
                            </UnitGrossPrice>
                            <TaxRate>
                            ...
                            <TaxAmount>
                                <xsl:value-of select="format-number($sumVAT - $sum, '.00')"/>
                            </TaxAmount>
                            <GrossAmount>
                                <xsl:value-of select="format-number($sumVAT, '.00')"/>
                            </GrossAmount>
                        </Line>
                    </xsl:for-each>
                </Packing-Sequence>
            ...
            <DespatchAdvice-Summary>
                ...
                <TotalLines>
                    <xsl:value-of select="count(//НомерСтроки)"/>
                </TotalLines>
				<TotalGoodsDespatchedAmount>
					<xsl:value-of select="sum(//Line-Item/Количество)"/>
				</TotalGoodsDespatchedAmount>
            </DespatchAdvice-Summary>
        </Document-DespatchAdvice>
    </xsl:template>
